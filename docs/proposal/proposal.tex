...
Agents generate and sign off both the compute datapath and the fabric endpoint
through all five gates with no human intervention. A bitstream produced from
agent-written RTL loads on one FPGA and passes its self-test. The simulated cluster
hosts Qwen3-0.6B and reports throughput within 15\% of the analytic prediction.
At least 100 automated tests pass from a single command.

\subsection{Silver}
Two boards are linked over the generated fabric. Ring all-reduce runs across real
hardware and produces results bit-identical to a single board, including under
injected bit errors. Measured end-to-end throughput reaches 40 tokens per second or
more on Qwen3-0.6B and is compared against the prediction. At least one block is
demonstrably fixed by an LLM reading tool output, with the transcript published.

\subsection{Gold}
Larger models hosted by adding boards, demonstrating that throughput holds roughly
constant as the model and the cluster grow together. A multi-agent swarm in which
an agent writes the verification as well as the RTL. A quantified study of
alternative interconnects, including photonic links. Optionally, the compute block
hardened to GDSII on an open process and submitted to a shared shuttle.
