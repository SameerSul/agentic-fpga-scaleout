...
\begin{ganttchart}[
  hgrid, vgrid,
  x unit=0.56cm, y unit title=0.5cm, y unit chart=0.40cm,
  ...
  \gantttitle{Project week}{24} \\
  \gantttitlelist{2,4,...,24}{2} \\
  \ganttgroup{A. Agentic generation and signoff (Sameer)}{1}{18} \\
  \ganttbar{Agent loop, spec derivation}{1}{4} \\
  \ganttbar{Place-and-route as fifth gate}{3}{8} \\
  \ganttbar{LLM iteration, multi-role agents}{7}{12} \\
  \ganttbar{Sizing against measured hardware}{13}{18} \\
  \ganttgroup{B. Fabric and collectives (Yax)}{1}{16} \\
  \ganttbar{Multi-agent architecture, survey}{1}{6} \\
  \ganttbar{Endpoint re-derived for real link}{9}{12} \\
  \ganttbar{Two-board framing bring-up}{11}{14} \\
  \ganttbar{Ring all-reduce on hardware}{13}{16} \\
  \ganttgroup{C. Physical implementation (Vaibhav)}{1}{14} \\
  \ganttbar{Vivado flow, reference bitstream}{1}{4} \\
  \ganttbar{Constraints, wrapper, RTL on board}{3}{6} \\
  \ganttbar{Post-route data into the profile}{7}{10} \\
  \ganttbar{Second board bring-up}{11}{14} \\
  \ganttgroup{D. Verification and physical link (Siddh)}{1}{20} \\
  \ganttbar{Bring-up and acceptance test plan}{1}{4} \\
  \ganttbar{Physical link characterization}{7}{12} \\
  \ganttbar{Coverage instrumentation}{13}{18} \\
  \ganttbar{Formal proofs}{17}{20} \\
  \ganttgroup{Shared}{19}{24} \\
  \ganttbar{Stretch objectives}{19}{22} \\
  \ganttbar{Integration, final report, demonstration}{21}{24} \\
  \ganttmilestone{Proposal}{2}
  \ganttmilestone{Bronze}{6}
  \ganttmilestone{Silver}{16}
  \ganttmilestone{Gold}{22}
\end{ganttchart}
\caption{Schedule across 24 weeks. The four module lanes run concurrently
throughout, with integration checkpoints at each two-week boundary. Diamonds mark
the deliverable tiers: Bronze at week 6, Silver at week 16, Gold at week 22.}
\label{fig:gantt}
\end{figure*}

The Bronze milestone lands at week 6, early enough that a working
specification-to-bitstream result exists well before the midpoint. Silver completes
at week 16, leaving roughly four weeks of float in the second half to absorb
hardware delays, which historically dominate schedule risk on projects of this
type. Gold work is scheduled last precisely so it can be dropped without
jeopardising anything already demonstrated.
